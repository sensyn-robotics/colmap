...
DATASET_PATH=$1 #data directory which has images dir, and poslog.csv.
WORKDIR=${DATASET_PATH}/work # output directory
mkdir -p ${WORKDIR}
...
colmap model_aligner --input_path ${WORKDIR}/sparse/0 --output_path ${WORKDIR}/sparse/${GEOREGIDIR} --ref_images_path ${DATASET_PATH}/campose.txt --robust_alignment 1 --robust_alignment_max_error 0.01
